...
export const CreateListingSchema = yup.object().shape({
    name: yup.string().min(3, `Name ${minLengthError}`).max(30, `Name ${maxLengthError}`),
    category: yup.string().min(3, `Category ${minLengthError}`).max(30, `category ${maxLengthError}`),

    image: yup.string().required('image is required!'),

    description: yup.string().min(3, `description ${minLengthError}`).max(30, `description ${maxLengthError}`),

    price: yup.number().min(30, `price ${minLengthError}`).max(30, `price ${maxLengthError}`),
